...
function switchAuthMode(mode) {
    ...
    if (activeForm) {
        const firstTabBtn = activeForm.querySelector('.tab-btn');
        if (firstTabBtn) switchUserType(firstTabBtn);
    }
}
...
async function loadLecturerData() {
    try {
        ...
        if (selectCodeEl) selectCodeEl.innerHTML = '<option value="">-- Select a Course --</option>';
        if (selectReportEl) selectReportEl.innerHTML = '<option value="">-- Select a Course to View Report --</option>';

        courses.forEach(course => {
            if (listEl) listEl.innerHTML += `<p><b>${course.course_code}:</b> ${course.course_title}</p>`;
            if (selectCodeEl) selectCodeEl.innerHTML += `<option value="${course.id}">${course.course_code} - ${course.course_title}</option>`;
            if (selectReportEl) selectReportEl.innerHTML += `<option value="${course.id}">${course.course_code} - ${course.course_title}</option>`;
        });
    } catch (error) {
...
function loginSuccess(token, userInfo) {
    ...
    if (currentUser.type === 'student') {
        ...
        if (welcomeEl) welcomeEl.textContent = `Welcome, ${currentUser.info.name}!`;
        navigate('student');
    } else if (currentUser.type === 'lecturer') {
        const welcomeEl = document.getElementById('lecturerWelcome');
        if (welcomeEl) welcomeEl.textContent = `Welcome, ${currentUser.info.name}!`;
        loadLecturerData();
        navigate('lecturer');
...
// Event Handlers - Setup after DOM loads
function setupEventHandlers() {
    ...
    if (courseForm) {
        courseForm.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                if (createCourseCard) createCourseCard.classList.add('hidden');
                loadLecturerData();
        ...
        generateCodeForm.addEventListener('submit', (e) => {
            ...
            navigator.geolocation.getCurrentPosition(async (position) => {
                ...
                try {
                    ...
                    if (codeDisplay) codeDisplay.textContent = `Code: ${result.code}`;
                } catch (error) { 
                    displayMessage('lecturerError', error.message); 
                }
            }, () => { 
    ...
    const courseSelectForReport = document.getElementById('courseSelectForReport');
    if (courseSelectForReport) {
        courseSelectForReport.addEventListener('change', async (e) => {
            ...
            try {
                if (sessionsContainer) sessionsContainer.innerHTML = '<div class="loading">Loading attendance sessions...</div>';
