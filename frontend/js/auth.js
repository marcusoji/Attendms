...
async function handleStudentFaceLogin() {
    ...
    // Step 1: Get the registered face scan path from the server
    const studentData = await apiFetch('/login/student', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ matNo })
    });

    // Step 2: Load the registered image from the server
    const registeredImage = await faceapi.fetchImage(`${API_URL}/${studentData.faceScanPath.replace(/\\/g, '/')}`);
    
    // Step 3: Create an image element from the live captured blob
    const liveImage = await faceapi.bufferToImage(faceScanBlob);

    // Step 4: Get face descriptors (the "fingerprint" of the face) for both images
    ...
    if (!registeredDetections || !liveDetections) {
        throw new Error("Could not detect a face in one of the images. Please try again with a clear picture.");
    }
    ...
    // A lower distance is a better match. 0.6 is a common threshold. We use 0.5 for higher accuracy.
